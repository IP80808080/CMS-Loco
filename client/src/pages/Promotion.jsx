...
export default function Promotion(user) {
  ...
  const handleUpdate = async (uid) => {
    try {
      const response = await axios.put(`${BASE_URL}/api/${uid}/rolensection`, {
        role: selectedRole,
  ...
  const handleDelete = async (uid) => {
    try {
      const userId = uid;
      await axios.delete(`${BASE_URL}/api/userdelete/${userId}`);
              ...
              >
                ...
                <div className="relative w-full">
                  ...
                  <input
                    ...
                    placeholder="Search either Username or ID"
                    required=""
                  />
                </div>
              </form>
                    ...
                    return (
                      ...
                      (uidString.toLowerCase().includes(searchLower) &&
                        (selectedFilters.Admin
                          ? user.role === "Admin"
                          : true) &&
                        (selectedFilters.Supervisor
                          ? user.role === "Supervisor"
                          : true) &&
                        (selectedFilters.Staff ? user.role === "Staff" : true))
                    );
                  })
                  .map((user) => (
                    <tr
                          ...
                          <Box sx={{ minWidth: 130 }}>
                            <FormControl fullWidth>
                              ...
                              >
                                ...
                                <MenuItem value={"Supervisor"}>JE/SSE</MenuItem>
