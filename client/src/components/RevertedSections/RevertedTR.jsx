...
export default function RevertedTR() {
  ...
  useEffect(() => {
    const fetchForms = async () => {
      try {
        ...
        const EMCResponse = await axios.get(
          `${BASE_URL}/api/revert/TR-EMC/${uid}`
        );
        setEMCFormsHistory(EMCResponse.data);

        // Fetch VCBSBAREVA forms
        const VCBSBAREVAResponse = await axios.get(
          `${BASE_URL}/api/revert/TR-VCBSBAREVA/${uid}`
        );
        setVCBSBAREVAFormsHistory(VCBSBAREVAResponse.data);
      } catch (error) {
        ...
        <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                {Array.isArray(VCBSBAREVAFormsHistory) &&
                  VCBSBAREVAFormsHistory.map((form) => (
                    <tr
                      key={form._id}
                    ...
                    >
                      ...
                      <th className="px-4 py-3 font-medium text-gray-900">
                        <div className="flex justify-center">
                          <Link
                            to={`/view-revert-VCBSBAREVA-form/${form.submittedDate}/${form.uid}`}
                ...
                {Array.isArray(EMCFormsHistory) &&
                  EMCFormsHistory.map((form) => (
                    <tr
                      key={form._id}
                      className="border-b dark:border-gray-700"
                    >
                      ...
                      <th className="px-4 py-3 font-medium text-gray-900">
                        <div className="flex justify-center">EMC</div>
                      </th>
                      <th className="px-4 py-3 font-medium text-gray-900">
                        <div className="flex justify-center">
                          {form.section}
                      ...
                      <th className="px-4 py-3 font-medium text-gray-900">
                        <div className="flex justify-center">
                          <Link
                            to={`/view-revert-EMC-form/${form.submittedDate}/${form.uid}`}
                            className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 border border-blue-700 rounded"
                          >
                            Re-submit
                          </Link>
