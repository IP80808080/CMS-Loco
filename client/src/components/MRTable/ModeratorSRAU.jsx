...
export default function ModeratorSRAU() {
  ...
  useEffect(() => {
    const fetchForms = async () => {
      try {
        // Fetch OCB forms
        const ocbResponse = await axios.get(`${BASE_URL}/api/daily/ocb`);
        setOcbForms(ocbResponse.data);

        // Fetch MRB forms
        const mrbResponse = await axios.get(`${BASE_URL}/api/daily/mrb`);
  ...
  const acceptOCBForm = async (formId) => {
    try {
      ...
      await axios.put(`${BASE_URL}/api/ocb/${formId}/accept`, {
        sign2: username,
      });
      // Reload OCB forms after accepting
      const response = await axios.get(`${BASE_URL}/api/daily/ocb`);
  ...
  const acceptMRBForm = async (formId) => {
    try {
      const { username } = await getUsername();
      await axios.put(`${BASE_URL}/api/mrb/${formId}/accept`, {
        sign2: username,
      });
      // Reload MRB forms after accepting
      const response = await axios.get(`${BASE_URL}/api/daily/mrb`);
